...
interface SheetContentProps
  ...
  /** If true, automatically adjusts side based on RTL/LTR direction */
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = 'right', directionAware = false, className, children, ...props }, ref) => {
  const { isRTL } = useDirection();

  // If directionAware is true, flip left/right based on direction
  let actualSide = side;
  if (directionAware && (side === 'left' || side === 'right')) {
    if (isRTL) {
      actualSide = side === 'left' ? 'right' : 'left';
    }
  }
