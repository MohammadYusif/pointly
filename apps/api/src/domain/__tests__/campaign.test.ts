...
describe('Campaign Entity', () => {
  ...
  it('should reject empty name', () => {
    expect(() => Campaign.create('merchant-1', '', 'desc', futureStart, futureEnd, 2.0)).toThrow(
      ValidationError,
    );
  });
  ...
  it('should reject end date before start date', () => {
    ...
    expect(() => Campaign.create('merchant-1', 'Test', 'desc', start, end, 2.0)).toThrow(
      ValidationError,
    );
  });
  ...
  it('should reject multiplier below 1.0', () => {
    expect(() =>
      Campaign.create('merchant-1', 'Test', 'desc', futureStart, futureEnd, 0.5),
    ).toThrow(ValidationError);
  });

  it('should reject multiplier above 5.0', () => {
    expect(() =>
      Campaign.create('merchant-1', 'Test', 'desc', futureStart, futureEnd, 6.0),
    ).toThrow(ValidationError);
  });
  ...
  it('should serialize to JSON', () => {
    const campaign = Campaign.create('merchant-1', 'Test', 'desc', futureStart, futureEnd, 2.0);
    const json = campaign.toJSON();

    expect(json.campaignId).toBeTruthy();
    expect(json.merchantId).toBe('merchant-1');
    expect(json.multiplier).toBe(2.0);
